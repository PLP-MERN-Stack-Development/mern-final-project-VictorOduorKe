...
router.post("/", async (req, res) => {
  try {
    const { user_id, phone, account_number, method, email, subscription } = req.body;

    if (!user_id || !phone || !email || !subscription) {
      return res.status(400).json({ message: "user_id, phone, email and subscription are required" });
    }
    ...
    let amount = 0;
    const pkg = subscription.toLowerCase().trim();
...
router.post("/webhook", express.json({ type: "*/*" }), async (req, res) => {
  try {
    ...
    if (event.event === "charge.success") {
      ...
      const pkg = payment.subscription?.toLowerCase().trim();
      let durationDays = 0;
